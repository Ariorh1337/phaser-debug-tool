...
export default function defineTexture(folder: any, obj: any) {
    ...
    const list1Opt = () => Object.keys(obj.texture.manager.list).map(a => ({ text: a, value: a }));
    const list2Opt = () => Object.keys(obj.texture.frames).map(a => ({ text: a, value: a }));
    ...
    const proxy = {
        ...
        set "texture.key"(value) {
            ...
            saved.list2 = textureFolder.addInput(proxy, "frame.name", { options: list2Opt(), label: "frame" });
        },
        get "frame.name"() {
    ...
    saved.list1 = textureFolder.addInput(proxy, "texture.key", { options: list1Opt(), label: "texture" });
