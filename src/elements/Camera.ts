...
export default function addCamera(
    ...
    if ((obj as any).deadzone) {
        ...
        folder.addMonitor((obj as any).deadzone, "width", {
            label: "deadzone width",
        });
        folder.addMonitor((obj as any).deadzone, "height", {
            label: "deadzone height",
        });
    }

    folder.addButton({ title: "Fade in" }).on("click", () => {
        (obj as any).fadeIn();
    });
    folder.addButton({ title: "Fade out" }).on("click", () => {
        (obj as any).fadeOut();
    });
    folder.addButton({ title: "Flash" }).on("click", () => {
        (obj as any).flash();
    });
    folder.addButton({ title: "Reset effects" }).on("click", () => {
        (obj as any).resetFX();
    });
    folder.addButton({ title: "Shake" }).on("click", () => {
        (obj as any).shake();
    });
